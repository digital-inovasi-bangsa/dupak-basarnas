...
class lap_keg_bln
{
	...
	public function tampilNilai($tahun,$bulan,$id_keg,$id_pegawai){		
		$query	= "SELECT sum(nilai) AS hasil FROM kegiatan_bulanan WHERE id_keg_bln = '$id_keg' AND bln_keg = '$bulan' AND thn_keg = '$tahun' AND '$id_pegawai' ";
	...
	public function countNilaiBln($tahun,$bulan,$bln1,$bln6,$id_keg,$id_pegawai){		
		$query	= "SELECT sum(nilai) AS hasilTambahbln FROM kegiatan_bulanan WHERE id_keg_bln = '$id_keg' AND '$id_pegawai' AND thn_keg = '$tahun' AND bln_keg BETWEEN '$bln1' AND  '$bln6' ";
		$query 	= $this->db->prepare($query);
		$query->execute();
		$result = $query->fetch(PDO::FETCH_OBJ);
		if($query->rowCount()>0){
			echo $result->hasilTambahbln;
		}else{
			echo "";
		}
	}
